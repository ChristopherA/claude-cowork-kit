---
name: BINDER-setup
description: Sets up the [BINDER] binder: a short interview, the Context documents, and the instructions to paste. Use for "set up my [BINDER] binder", "[BINDER] setup".
---

# Setup

Build the [BINDER] binder from a few answers. Create what a task can create, the Context documents, and hand back what only the reader can paste, the project instructions. Read the folder if one is connected; do not write into it during setup.
...
1. **The folder.** Where the files for this binder are on their computer, or whether there are none yet. If they name a folder, confirm you can reach it; if you cannot, say so and continue from their description.
2. [**THE QUESTION THAT SHAPES THE FIRST CONTEXT DOCUMENT.**]
3. [**THE QUESTION THAT SHAPES THE SECOND CONTEXT DOCUMENT, IF ANY.**]

## Create the Context documents

Create [`FIRST-DOCUMENT.md`] as a Context document: [WHAT IT HOLDS, IN ONE SENTENCE]. Create [`SECOND-DOCUMENT.md`] the same way. Read each back and confirm it exists as a Context document. If Context documents cannot be created from a task, say so, show the texts in full, and say the rest depends on them.
...
Report what exists now: the Context documents by name and the two fields waiting for a paste. Name one day-one check: from the phone, ask something a Context document alone can answer.
...
It does not write into the folder. It does not set any setting. It does not paste anything anywhere; it hands text back. It does not hand back the account-wide instructions; any of the kit's own binder setups, or the core plugin's setup, does.
