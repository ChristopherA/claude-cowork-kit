...
For a reader comfortable editing files, or a contributor. A reader who wants a project of their own does not need this: the core plugin's `cowork-new-binder` skill designs the project and hands back the instructions text, and that text with a few project docs is the whole project. This folder is the extra step that turns such a design into a plugin whose setup runs from a phrase.
...
- `skills/BINDER-setup/references/project-instructions.md`: the project's instructions block, in the kit's shape (what the project is for, what belongs elsewhere, where things live, how to answer, what Claude is not). The core plugin's `cowork-new-binder` skill drafts this with you.
