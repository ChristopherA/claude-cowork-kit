# Workstream Close -- the failures behind the moves
...
A `maintain` workstream, or any whose criteria are standing health conditions rather than an exit, has no closure milestone to reach. A request to close one is usually right about the need and wrong about the operation: the file has grown unreadable and ending it is the reflex.
...
**This is the last run.** Nothing may be deferred to a later pass, because there is no later pass: an insight not routed now dies in the tag. A disposition that names a future task in THIS workstream is not a disposition here — extract's periodic mode tolerates one, and closure does not. Read with extract's outward sweep, which routes findings owed elsewhere to that workstream's Backlog with provenance, the two texts once refused a routed entry: the first closure under the split skills carried three Learnings and an Open Question ROUTED to open tasks in another live workstream, each destination line naming the source. Those ARE dispositions, and `cites` on the destination is the check.
...
All three reads are now `workstream-record.py git <root> --tags`: it reports a missing remote as such rather than as every tag dangling, takes the one network call only under `--tags`, and filters the `^{}` dereference itself; its suite fires each of the cases above.
