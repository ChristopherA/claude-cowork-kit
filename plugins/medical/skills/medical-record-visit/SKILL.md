...
If there is a transcript, clean it before anything is drawn from it: name each speaker in full the first time and by role after; keep the flow of speech and drop most of the filler; fix medical terms and proper nouns; join consecutive turns by the same speaker; note where a gesture or a pointed-at result loses its meaning in text. Save the cleaned transcript into the folder beside the visit's other records, named by date and role, on a yes. A scanned or portal document that the reader wants as text is saved the same way, with its key findings marked, on its own yes.

## Step 1: the timeline line
...
Draft the visit note that goes in the folder, a dated file named by date and role in the form the folder already uses. It records this visit and points at the standing files rather than repeating them. Its parts, only those with content: a header (date, place, clinician's role, who attended, the recording if any, the kind of visit); what the clinician found, said and decided, with the important statements as exact quotes; where a point was contested or matters to an insurer or a claim, exactly what the clinician said and did not say about it, quoted, with no gloss; what was ordered, recommended or changed, as a table when there is more than one item (what it is, how it works as the clinician put it, what was said about success, recovery and coverage); any therapy or rehabilitation discussed; the reader's functional status at the visit; action items as checkboxes; and a table of every document from this visit by file name.
