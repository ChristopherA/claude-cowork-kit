...
If there is a transcript, clean it before anything is drawn from it with the core's transcript skill (`clean up this transcript`), at the level that keeps every clinical term and proper noun, noting where a gesture or a pointed-at result loses its meaning in text; if that skill is not installed, do the same by hand, speakers named, filler dropped, terms fixed. Save the cleaned transcript into the folder beside the visit's other records, named by date, `YYYY-MM-DD`, and role, on a yes. A scanned or portal document that the reader wants as text is saved the same way, with its key findings marked, on its own yes.
