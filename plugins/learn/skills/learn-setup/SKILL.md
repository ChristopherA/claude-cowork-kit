...
Report what exists now: the three Context documents by name, and the two fields waiting for a paste, the one-line description under the title and the Instructions panel. Then name the day-one checks, in order: in this project, ask Claude a direct question about the subject and see whether it asks first, as the project instructions say; from the phone with the computer closed, ask where the progress record says you are; then say `next lesson`. Do not run them; the reader does. Say that the first lesson is `next lesson`, and that `quiz me` works once there is something to quiz. End with one word for completeness: full, partial (with what is missing), or minimal (stopped early).
