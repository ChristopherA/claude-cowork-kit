...
The Claude Cowork Kit's core: a setup interview that hands back the account instructions and says which project plugins to install next, plus ten routines for any project: deciding, interviewing, explaining again, questions for others, confidence, premortem, postmortem, where was I, wrap-up, and a new project.

Install this plugin first, then start a task and say `set up the kit`. The setup skill asks a few questions, hands back the account-wide instructions to paste, and says which project plugin to install next. The other ten skills work in any project.
