...
- **Learning, the week, money, medical:** each is its own plugin with its own setup, installed and run the same way. Where the kit has not yet published one, say so, and point the reader at the kit's explainer, which carries each project's instructions block in that project's section to paste by hand; do not invent a plugin name.
