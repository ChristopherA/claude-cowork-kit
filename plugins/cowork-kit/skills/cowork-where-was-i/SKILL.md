...
1. `map.md`, the "what I'm working on now" and "open threads" sections. This is the reader's own statement of what matters this month.
2. `inbox.md`: how many items are waiting, and how old the oldest looks.
3. If the folder is reachable, the notes and threads changed most recently: their names and, for the top few, their first lines. If the folder is not reachable, say so; the recommendation then comes from the description and the inbox, which is enough for a phone.
...
- **An open thread with fresh movement.** A thread named in `map.md` whose note changed in the last few days is the most likely place the reader's attention was. Continuing it is the default recommendation.
...
- **A stale description.** If the newest notes and threads do not match what `map.md` says the reader is working on, the next step is to fix the description, because every session after this one starts from it.
