...
3. **What gap let it happen?** Not the moment but the system: a rule that lives only in someone's head, a convention the project's Context documents do not state, a step the routine skips, a place where two files can drift apart.
4. **What prevents it next time?** One change, the smallest that closes the gap named in question three. Prefer a sentence in the binder's rules or description document (`rules.md` or `map.md` in the research binder, the priorities, mission, categories or questions document elsewhere) over a new procedure, and prefer a check that fires anyway over one that depends on remembering. The Instructions field is closed to a task and holds only what should never change, so a change lands there only when the reader wants a boundary moved.

## Propose the change

Show the exact wording and where it goes: which section of which Context document, or which note. Ask for the yes through the app's question control, recommended option first, and write it only after the yes; read the document back and confirm the change landed. If the right fix is in the project instructions, which only the reader can edit, hand them the sentence to paste and say so.
