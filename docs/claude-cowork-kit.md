...
**The plugins.** The kit's skills ship as plugins, one per project plus the core. The README's table lists every plugin with its skills in a phrase, and each plugin's own README lists the phrases that trigger them. Install them one at a time, and run each plugin's setup before installing the next.
