...
- **`medical-setup`**, `set up my medical project`. The privacy floor first, then three questions, then the two project docs and the instructions to paste. If the folder is reachable and holds none of the standing files, it offers to create them empty, with a heading each.
...
The setup creates two project docs: `questions.md`, the running list of questions for appointments, and `timeline.md`, the bare list of visit dates by clinician role. Neither holds anything clinical. The general steps are in the explainer under Setting up a project, and the install paths in the repository's README; for this project:
