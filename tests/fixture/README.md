...
A small notes folder and the two project docs that go with it, for testing the kit's skills in Cowork before a reader's real notes are involved. The setup skill creates `rules.md`, `map.md` and `inbox.md` as project docs from the reader's answers; `context-documents/` holds the three the test's answers should produce, for comparison. `research-folder/` is copied by hand into the connected folder after setup, so the check skills have something to find.
